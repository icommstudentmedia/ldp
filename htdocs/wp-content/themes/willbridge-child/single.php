...
?>
   <div class="entry">
      ...
      /* If the post is a "Video" post, display the message about resizing. It will go through the $page_categories that has
         all of the different categories for the post as objects in an array. The foreach loop will then go through however 
         many objects are returned and look to see if the category name (cat_name) is equal to "Video" and if it is, will 
         then display the message and break out of the loop. */
      $page_categories = get_the_category(get_the_ID());
      foreach ($page_categories as $page_category) {
         if (($page_category->cat_name) == "Video") {
            echo '<p id="resize_message">* If the video did not display correctly, please resize your browser to get the 
